...
if (isset($_GET['chempadd']) && $user['des'] > 1 && $user['id'] != $_GET['chempadd']) {
    if ($mc->query("SELECT `id_clan` FROM `users` WHERE `id`='" . $_GET['chempadd'] . "'")->fetch_array(MYSQLI_ASSOC)['id_clan'] == $user['id_clan']) {
        ...
        if($CountClan['COUNT(*)'] < 3) {
            ...
            showClanModal('Чемпион успешно назначен', 'Согласиться', 'main.php');
        } else {
            // Показываем модальное окно с сообщением о лимите
            showClanModal('Достигнут лимит чемпионов', 'Вернуться', 'main.php');
        }
        exit(0);
    } else {
    ...
    // Установка переменных для сообщения
    $message = 'Вы не можете назначить чемпионом самого себя';
    
    // Показываем модальное окно с ошибкой
    showClanModal($message, 'Вернуться', 'main.php');
    exit(0);
} else {
    // Установка переменных для сообщения
    $message = 'Недостаточно полномочий для назначения чемпиона';
