...
// Обработка назначения десятника
if (isset($_GET['desadd']) && $user['des'] > 2 && $user['id'] != $_GET['desadd']) {
    if ($mc->query("SELECT `id_clan` FROM `users` WHERE `id`='" . $_GET['desadd'] . "'")->fetch_array(MYSQLI_ASSOC)['id_clan'] == $user['id_clan']) {
        $CountClan = $mc->query("SELECT COUNT(*) FROM `users` WHERE (`id_clan` = '".$user['id_clan']."' AND `des` = '2')")->fetch_array(MYSQLI_ASSOC);
        if($CountClan['COUNT(*)'] < 3) {
            $mc->query("UPDATE `users` SET `des`='2' WHERE `id`= '" . $_GET['desadd'] . "'");
            // Показываем модальное окно вместо редиректа
            showClanModal('Десятник успешно назначен', 'Согласиться', 'main.php');
        } else {
            ?>
            <div class="clan_container">
                <div class="clan_message">
                    <i class="fas fa-exclamation-triangle"></i> Достигнут лимит десятников
                </div>
...
} elseif (isset($_GET['desadd']) && $user['des'] > 2 && $user['id'] == $_GET['desadd']) {
    ?>
    <div class="clan_container">
        <div class="clan_message error">
            <i class="fas fa-exclamation-circle"></i> Вы не можете назначить десятником самого себя
        </div>
        <a class="clan_button" onclick="showContent('main.php')">
            <i class="fas fa-arrow-left"></i> Вернуться
        </a>
    ...
    <div class="clan_container">
        <div class="clan_message error">
            <i class="fas fa-exclamation-circle"></i> Недостаточно полномочий для назначения десятника
        </div>
        <a class="clan_button" onclick="showContent('main.php')">
            <i class="fas fa-arrow-left"></i> Вернуться
        </a>
