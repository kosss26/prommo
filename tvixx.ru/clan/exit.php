...
require_once('../functions/clan_functions.php');
...
// После выполнения выхода игрока
if (isset($_GET['exit'])) {
    $clan_id = $user['id_clan']; // Сохраняем ID клана перед обновлением
    
    // Обновляем данные пользователя при выходе из клана
    $mc->query("UPDATE `users` SET `des`='0',`reit`='0',`id_clan`='0',`id_new_clan`='0' WHERE `id`= '" . $user['id'] . "'");
    
    // Проверяем и удаляем клан, если он пустой
    check_and_delete_empty_clan($clan_id);
    
    // Показываем модальное окно вместо редиректа
    showClanModal('Вы успешно покинули клан', 'Согласиться', 'main.php');
    
    exit(0);
} else {
    // Используем модальное окно вместо обычного сообщения
    showClanModal('Ошибка доступа к странице', 'Вернуться', 'main.php');
    exit(0);
} 