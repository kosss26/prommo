...
$timeThis = time();
//запишем время старта боя
$battle_start_time = $timeThis;
$commax = 32;
$commin = 1;
//перебираем все уровни турниров
for ($i0 = 0; $i0 < count($arr_lvl); $i0++) {
    //текущий уровень мин макс
    $minLevel = $arr_lvl[$i0][0];
    $maxLevel = $arr_lvl[$i0][1];
    //создадим случайный идентификатор боя
    $battle_id = rand(0, $timeThis) . rand(0, $timeThis) . rand(0, $timeThis);
    //проверим игроков зарегистрированых на вжв 3
    if ($mc->query("SELECT * FROM `huntb_list` WHERE `level` >= '$minLevel' && `level` <= '$maxLevel' && `type`='3'")->num_rows >= $commin) {
        //получим зарегистрированных игроков
        $arrU = $mc->query("SELECT * FROM `huntb_list` WHERE `level` >= '$minLevel' && `level` <= '$maxLevel' && `type`='3'")->fetch_all(MYSQLI_ASSOC);
        //переберем их
        for ($i = 0; $i < count($arrU); $i++) {
            //получим данного игрока
            $users = $mc->query("SELECT * FROM `users` WHERE `id` = '" . $arrU[$i]['user_id'] . "'")->fetch_array(MYSQLI_ASSOC);
            if ($i == 0) {
                //если запись первая то запишем ему название боя
                hero_add($i + 1, "выживание", $users, $battle_id, $battle_start_time, 3);
            } else {
                //или добавим игроков
                hero_add($i + 1, "", $users, $battle_id, $battle_start_time, 3);
            }
        }
        //количество недостающих ботов
        $botNum = 6 - count($arrU);
        //если больше 0 
        if ($botNum > 0) {
            //сгенерировать ботов (массив имен,количество)
            $arrbotpar = genbotpar($names, $botNum);
            //перебираем ботов
            for ($i = 0; $i < count($arrbotpar[0]); $i++) {
                //если они есть то создаем новых
                bot_add($arrbotpar[0][$i] . "[БОТ]", $i + count($arrU) + 1, $arrbotpar[1][$i], $arrbotpar[2][$i], $users['level'], $battle_id, $battle_start_time, 3);
            }
        }
        //сносим список зарегистрированных игроков
        $mc->query("DELETE FROM `huntb_list` WHERE `level` >= '$minLevel' && `level` <= '$maxLevel' && `type`='3'");
    }
}
