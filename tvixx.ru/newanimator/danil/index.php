    ...
    <body >
        ...
        <script>
            ...
            function SloyUpDown(Sloy, Sloyid) {
                if (Sloy === 'up') {
                    if (Sloyid > 0) {
                        var predSloy = spriteData[0][nanim][0][Sloyid - 1].slice();
                ...
                if (Sloy === 'down') {
                    if (Sloyid < spriteData[0][nanim][0].length - 1) {
                        var predSloy = spriteData[0][nanim][0][Sloyid + 1].slice();
                        spriteData[0][nanim][0][Sloyid + 1] = [];
                        spriteData[0][nanim][0][Sloyid + 1] = spriteData[0][nanim][0][Sloyid].slice();
                        spriteData[0][nanim][0][Sloyid] = [];
            ...
            function Animations() {
                ...
                for (var i = 0; i < spriteData.type.length; i++) {
                    animations.innerHTML += "<input type='button' value='" + spriteData.type[i] + "'onclick = 'nanim = " + i + ";OpenJsonCode();count = 0;cadr = stop;'>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type='button' onclick='DeleteAnims(" + i + ");' value='удалить'><br>";
                }
            ...
            function AddElement() {
                ...
                for (var i = 0; i < spriteData[0][nanim].length; i++) {
                    spriteData[0][nanim][i][idelem] = [50, 100, 50, 50, 50, 50, 50, 50, 0, "Имя"];
                }
                redrawElements();
            }
            function DeleteElement(e) {
                //создаем новый элемент во всех кадрах
                for (var i = 0; i < spriteData[0][nanim].length; i++) {
                    delete spriteData[0][nanim][i].splice(e, 1)
                    ;
                }
                redrawElements();
            }
            function redrawElements() {
                ...
                for (var i = 0; i < spriteData[0][nanim][0].length; i++) {
                    $("#elements").append('<div><input type="text" onchange="setName(' + i + ',$(this).val());" value="' + spriteData[0][nanim][0][i][9] + '">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="button" onclick="DeleteElement(' + i + ');" value="удолить"><br><input type="button" onclick="EditMask(' + i + ');" value="Маска"><input type="button" onclick="EditPos(' + i + ');" value="Позиция"><input type="button" onclick="SloyUpDown(\'up\',' + i + ');" value="&#9650"><input type="button" onclick="SloyUpDown(\'down\',' + i + ');" value="&#9660">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</div><hr>');
                }
            }
